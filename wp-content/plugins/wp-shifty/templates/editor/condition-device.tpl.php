...
?>
<div data-condition-editor="device" data-condition-id="%ID" class="wpshifty-hidden">
      <input type="hidden" name="condition[%ID][type]" value="device">
      ...
      <div class="wpshifty-nice-select-wrapper">
            ...
            <ul class="wpshifty-nice-select">
                  <?php foreach (WP_Shifty_Helper::get_devices() as $key => $device):?>
                        <li>
                              <input type="checkbox" id="wps_label_%ID_<?php echo esc_attr($key);?>" name="condition[%ID][devices][]" value="<?php echo esc_attr($key);?>"<?php WP_Shifty_Editor::maybe_checked($_condition, 'devices', $key, 'in');?>>
                              <label for="wps_label_%ID_<?php echo esc_attr($key);?>"><?php echo $device;?></label>
                        </li>
                  <?php endforeach;?>
                  <li class="wpshifty-nice-select-noresult wpshifty-hidden"><?php esc_html_e('No result found', 'wp-shifty');?></li>
            </ul>
      </div>
      <div class="wpshifty-condition-info"><?php esc_html_e('The condition is met if visitor\'s device is one of the selected above.', 'wp-shifty');?></div>
