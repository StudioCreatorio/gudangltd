...
?>
<div data-condition-editor="archive" data-condition-id="%ID" class="wpshifty-hidden">
      <input type="hidden" name="condition[%ID][type]" value="archive">
      ...
      <div class="wpshifty-nice-select-wrapper">
            ...
            <ul class="wpshifty-nice-select">
                  <?php foreach (WP_Shifty_Helper::get_taxonomies() as $tax):?>
                        <li>
                              <input type="checkbox" id="wps_label_%ID_<?php echo esc_attr($tax->name);?>" name="condition[%ID][archive][]" value="<?php echo esc_attr($tax->name);?>"<?php echo (isset($_condition->{'archive'}) && in_array($tax->name, $_condition->{'archive'}) ? ' checked' : '');?>>
                              <label for="wps_label_%ID_<?php echo esc_attr($tax->name);?>"><?php echo $tax->label;?></label>
                        </li>
                  <?php endforeach;?>
                  <li class="wpshifty-nice-select-noresult wpshifty-hidden"><?php esc_html_e('No result found', 'wp-shifty');?></li>
            </ul>
      </div>
      <div class="wpshifty-condition-info"><?php esc_html_e('The condition is met if the current page is one of these archive pages.', 'wp-shifty');?></div>
