...
?>
<div data-condition-editor="page" data-condition-id="%ID" class="wpshifty-hidden">
      <input type="hidden" name="condition[%ID][type]" value="page">
      ...
      <div class="wpshifty-nice-select-wrapper">
            ...
            <ul class="wpshifty-nice-select">
                  <?php foreach (get_posts(array('post_type' => 'page', 'post_status' => 'publish', 'posts_per_page' => -1, 'order' => 'asc', 'orderby' => 'post_title')) as $page):?>
                        <li>
                              <input type="checkbox" id="wps_label_%ID_<?php echo esc_attr($page->ID);?>" name="condition[%ID][pages][]" value="<?php echo esc_attr($page->ID);?>"<?php WP_Shifty_Editor::maybe_checked($_condition, 'pages', $page->ID, 'in');?>>
                              <label for="wps_label_%ID_<?php echo esc_attr($page->ID);?>"><?php echo get_the_title($page);?></label>
                        </li>
                  <?php endforeach;?>
                  <li class="wpshifty-nice-select-noresult wpshifty-hidden"><?php esc_html_e('No result found', 'wp-shifty');?></li>
            </ul>
      </div>
      <div class="wpshifty-condition-info"><?php esc_html_e('The condition is met if the current page is one of these pages.', 'wp-shifty');?></div>
