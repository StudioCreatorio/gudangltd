...
?>
<div data-condition-editor="post-type" data-condition-id="%ID" class="wpshifty-hidden">
      <input type="hidden" name="condition[%ID][type]" value="post-type">
      ...
      <div class="wpshifty-nice-select-wrapper">
            ...
            <ul class="wpshifty-nice-select">
                  <?php foreach (WP_Shifty_Helper::get_public_post_types('page') as $post_type):?>
                        <li>
                              <input type="checkbox" id="wps_label_%ID_<?php echo esc_attr($post_type);?>" name="condition[%ID][post-types][]" value="<?php echo esc_attr($post_type);?>"<?php WP_Shifty_Editor::maybe_checked($_condition, 'post-types', $post_type, 'in');?>>
                              <label for="wps_label_%ID_<?php echo esc_attr($post_type);?>"><?php echo get_post_type_object($post_type)->labels->singular_name;?></label>
                        </li>
                  <?php endforeach;?>
                  <li class="wpshifty-nice-select-noresult wpshifty-hidden"><?php esc_html_e('No result found', 'wp-shifty');?></li>
            </ul>
      </div>
      <div class="wpshifty-condition-info"><?php esc_html_e('The condition is met if the current page has one of these post types (archive pages included).', 'wp-shifty');?></div>
