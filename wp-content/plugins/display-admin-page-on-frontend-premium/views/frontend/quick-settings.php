...
<form class="vg-frontend-admin-quick-settings">
	...
	if (dapof_fs()->is_plan__premium_only('platform', true)) {
		if (VG_Admin_To_Frontend_Obj()->is_master_user() && is_multisite()) {
			$admin_content_site_id = (int) WPFA_Global_Dashboard_Obj()->get_site_id_for_admin_content();
			...
			<div class="field">
				<label>
					<?php _e('Preview site:', VG_Admin_To_Frontend::$textname); ?> <a href="#"  data-tooltip="down" aria-label="<?php esc_attr_e('By default, we show content from the current subsite for your customizations. But you can show content from a different site if you want to customize elements that depend on the other site\'s theme.', VG_Admin_To_Frontend::$textname); ?>">(?)</a>
				</label>
				<select name="site_id_for_preview">
					<?php
					foreach ($network_sites as $site_id => $site_url) {
						if (empty($site_url)) {
							$site_url = $site_id;
						}
						?>
						<option value="<?php echo (int) $site_id; ?>" <?php selected($admin_content_site_id, (int) $site_id); ?>><?php echo sanitize_text_field($site_url); ?></option>
						<?php
					}
					?>
				</select>
	...
	<div class="field">
		...
		<select name="page_template">
			...
			foreach ($templates as $template_filename => $template_name) {
				?>
				<option value="<?php echo esc_attr($template_filename); ?>" <?php selected($template_filename, $current_template); ?>><?php echo sanitize_text_field($template_name); ?></option>
				<?php
			}
			?>
		</select>
		...
		<?php if (!empty($menus)) { ?>
			<select name="menu">
				...
				foreach ($menus as $menu) {
					?>
					<option value="<?php echo (int) $menu->term_id; ?>"><?php echo sanitize_text_field($menu->name); ?></option>
					<?php
				}
				?>
			</select>
