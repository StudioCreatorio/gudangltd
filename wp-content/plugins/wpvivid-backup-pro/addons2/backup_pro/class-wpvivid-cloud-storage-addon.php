    ...
    public function clean_remote_schedule_event($backup_count=0,$db_count=0)
    {
        ...
        foreach ($remoteslist as $key=>$remote_option)
        {
            ...
            try
            {
                ...
                {
                    $backup_count=$this->get_backup_retain_count('Manual',$remote_option,true);
                    $db_count=$this->get_backup_db_retain_count('Manual',$remote_option,true);
                    do_action('wpvivid_schedule_scan_remote_backup', $key, 'Manual', $backup_count, $db_count);

                    $backup_count=$this->get_backup_retain_count('Cron',$remote_option,true);
                    $db_count=$this->get_backup_db_retain_count('Cron',$remote_option,true);
