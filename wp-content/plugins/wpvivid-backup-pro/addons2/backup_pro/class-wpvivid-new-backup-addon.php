...
class WPvivid_New_Backup_Page_addon
{
    ...
    public function __construct()
    {
        ...
        //test rollback
        //add_filter( 'upgrader_pre_install', array( $this, 'test_rollback' ), 10, 2 );
        //
    ...
    /*
    public function test_rollback( $response, $plugin )
    {
        ...
        $theme = isset( $plugin['theme'] ) ? $plugin['theme'] : '';
        ...
        if(!empty($plugin))
        {
            ...
            if($dir=='.')
            {
                $dir=WP_PLUGIN_DIR.DIRECTORY_SEPARATOR.$plugin;
            }
            else
            {
                $dir=WP_PLUGIN_DIR.DIRECTORY_SEPARATOR.$dir;
            }

            $dir=$this->transfer_path($dir);
            ...
            $path=WP_CONTENT_DIR.DIRECTORY_SEPARATOR.WPvivid_Setting::get_backupdir().DIRECTORY_SEPARATOR.'rollback.zip';
            $pclzip=new WPvivid_PclZip($path);
            $pclzip -> add($dir,WPVIVID_PCLZIP_OPT_REMOVE_PATH,$replace_path,WPVIVID_PCLZIP_OPT_NO_COMPRESSION,WPVIVID_PCLZIP_OPT_TEMP_FILE_THRESHOLD,16);
        }
