    ...
    public function admin_load_themes()
    {
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($)
            {
                ...
                {
                    location.href='<?php echo apply_filters('wpvivid_white_label_page_redirect', 'admin.php?page=wpvivid-backup', 'wpvivid-backup'); ?>&auto_backup=1&backup=themes&slug='+args.slug;
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($)
            {
                ...
                jQuery(document).on('wp-plugin-bulk-update-selected', function(event)
                {
                    ...
                    var url = 'admin.php?page=<?php echo strtolower(apply_filters('wpvivid_white_label_slug', WPVIVID_PRO_PLUGIN_SLUG)).'-backup'; ?>&auto_backup=1&backup=plugin';
                    ...
                    itemsSelected.each( function( index, element )
                    {
                        ...
                        // Add it to the queue.
                        var input=$('<input type="checkbox" name="checked[]" value="'+$itemRow.data( 'plugin' )+'" checked/>');
                ...
                $('tr.plugin-update-tr ').on('click', 'a', function(event)
                {
                    ...
                    {
                        var $message   = $( event.target ), $pluginRow = $message.parents( 'tr' );
                        if($pluginRow.data( 'plugin' ) !== 'wpvivid-backuprestore/wpvivid-backuprestore.php' && $pluginRow.data( 'plugin' ) !== 'wpvivid-backup-pro/wpvivid-backup-pro.php') {
                            location.href='<?php echo apply_filters('wpvivid_white_label_page_redirect', 'admin.php?page=wpvivid-backup', 'wpvivid-backup'); ?>&auto_backup=1&backup=plugin&slug='+$pluginRow.data( 'plugin' );
                            wpvivid_updater.ajaxLocked=true;
                        }
                    }
                });
